...
<div class="lc-page max-w-5xl">
    <section class="lc-page-header">
        <div>
            ...
            <h1 class="lc-title">Editar proveedor</h1>
            <p class="lc-subtitle">Actualiza la informacion comercial y operativa para mantener compras y almacen sincronizados.</p>
        </div>
    ...
    <section class="lc-card overflow-hidden">
        <div class="lc-card-header">
            <div>
                <h2 class="lc-section-title">Proveedor #{{ $proveedor->id }}</h2>
                <p class="lc-section-subtitle">Modifica solo lo necesario para preservar la trazabilidad de compras historicas.</p>
            </div>
        </div>

        <form method="POST" action="{{ route('proveedores.update', $proveedor->id) }}" class="lc-card-body space-y-6" x-data="{ submitting: false }" x-on:submit="submitting = true">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="lc-field">
                    <label for="tiempo_entrega_dias" class="lc-label">Tiempo de entrega (dias)</label>
                    <input id="tiempo_entrega_dias" name="tiempo_entrega_dias" type="number" min="1" max="120" value="{{ old('tiempo_entrega_dias', $proveedor->tiempo_entrega_dias) }}" class="lc-input">
                    @error('tiempo_entrega_dias')<p class="lc-help text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="lc-field">
                    <label for="dias_credito" class="lc-label">Credito (dias)</label>
                    <input id="dias_credito" name="dias_credito" type="number" min="0" max="365" value="{{ old('dias_credito', $proveedor->dias_credito) }}" class="lc-input">
