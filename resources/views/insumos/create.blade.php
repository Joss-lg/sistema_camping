...
<div class="container mx-auto px-4 py-6 space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-800">Crear insumo</h1>
            <p class="text-slate-500 text-sm mt-1">Registra un nuevo insumo en el catalogo de inventario.</p>
        </div>
    ...
    <section class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <form method="POST" action="{{ route('insumos.store') }}" class="space-y-5">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex flex-col gap-1.5">
                    <label class="text-sm font-semibold text-slate-600">Codigo</label>
                    <input type="text" name="codigo_insumo" value="{{ old('codigo_insumo') }}" class="border border-slate-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-green-500 outline-none">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" class="border border-slate-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-green-500 outline-none">
