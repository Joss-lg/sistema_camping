...
<div class="lc-page max-w-5xl">
    <section class="lc-page-header">
        <div>
            ...
            <h1 class="lc-title">Editar ubicacion de almacen</h1>
            <p class="lc-subtitle">Actualiza datos fisicos y capacidad sin romper la trazabilidad de inventario.</p>
        </div>
    ...
    <section class="lc-card overflow-hidden">
        <div class="lc-card-header">
            <div>
                <h2 class="lc-section-title">Ubicacion #{{ $ubicacion->id }}</h2>
                <p class="lc-section-subtitle">Ajusta identificacion, estado y capacidad de esta ubicacion.</p>
            </div>
        </div>

        <form method="POST" action="{{ route('almacenes.update', $ubicacion->id) }}" class="lc-card-body space-y-6" x-data="{ submitting: false }" x-on:submit="submitting = true">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div class="lc-field">
                    ...
                    <input id="codigo_ubicacion" name="codigo_ubicacion" type="text" value="{{ old('codigo_ubicacion', $ubicacion->codigo_ubicacion) }}" required class="lc-input" maxlength="20">
                    @error('codigo_ubicacion')<p class="lc-help text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="lc-field">
                    <label for="nombre" class="lc-label">Nombre</label>
                    <input id="nombre" name="nombre" type="text" value="{{ old('nombre', $ubicacion->nombre) }}" required class="lc-input" maxlength="100">
                ...
                <div class="lc-field">
                    ...
                    <select id="tipo" name="tipo" required class="lc-select">
                        @foreach ($tiposCatalogo as $itemTipo)
                            <option value="{{ $itemTipo }}" @selected(old('tipo', $ubicacion->tipo) === $itemTipo)>{{ $itemTipo }}</option>
                        @endforeach
                    </select>
                ...
                <div class="lc-field">
                    ...
                    <select id="activo" name="activo" required class="lc-select">
                        <option value="1" @selected((string) old('activo', (int) $ubicacion->activo) === '1')>Activo</option>
                        <option value="0" @selected((string) old('activo', (int) $ubicacion->activo) === '0')>Inactivo</option>
                    </select>
                ...
                <div class="lc-field">
                    ...
                    <input id="seccion" name="seccion" type="text" value="{{ old('seccion', $ubicacion->seccion) }}" class="lc-input" maxlength="50">
                    @error('seccion')<p class="lc-help text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="lc-field">
                    <label for="estante" class="lc-label">Estante</label>
                    <input id="estante" name="estante" type="text" value="{{ old('estante', $ubicacion->estante) }}" class="lc-input" maxlength="20">
                    @error('estante')<p class="lc-help text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="lc-field">
                    <label for="nivel" class="lc-label">Nivel</label>
                    <input id="nivel" name="nivel" type="text" value="{{ old('nivel', $ubicacion->nivel) }}" class="lc-input" maxlength="20">
                ...
                <div class="lc-field">
                    ...
                    <input id="capacidad_maxima" name="capacidad_maxima" type="number" min="0" step="0.0001" value="{{ old('capacidad_maxima', $ubicacion->capacidad_maxima) }}" class="lc-input" placeholder="Opcional">
                ...
                <div class="lc-field md:col-span-2">
                    ...
                    <input id="capacidad_actual" name="capacidad_actual" type="number" min="0" step="0.0001" value="{{ old('capacidad_actual', $ubicacion->capacidad_actual) }}" class="lc-input">
